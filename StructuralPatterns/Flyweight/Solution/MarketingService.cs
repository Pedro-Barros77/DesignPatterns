...
namespace Flyweight.Solution
{
    public static class MarketingService
    {
        ...
        public static async Task PopulateCityWithBillboards()
        {
            ...
            WriteColored("", 1);
            WriteColored(new TextItem("Iniciando teste de stress com ", ConsoleColor.Green), new($"{goldenCount + mediumCount + stickerCount}", ConsoleColor.Red), new(" Billboards", ConsoleColor.Green, 2));
            ...
            BillboardFlyweightFactory.PrintFlyweightsCount();
            WriteColored("", 5);
        ...
        private static async Task<Billboard> CreateGoldenBillboard(Vector3 pos)
        {
            ...
            var flyWeight = BillboardFlyweightFactory.Create(new(20, 20), image, Color.Gold, true, true);
            return new Billboard(pos, flyWeight);
        }

        private static async Task<Billboard> CreateSmallSticker(Vector3 pos)
        {
            byte[] image = await LoadImage();
            var flyWeight = BillboardFlyweightFactory.Create(new(0.5f, 0.5f), image, Color.White, false, false);
            return new Billboard(pos, flyWeight);
        }
        ...
        {
            byte[] image = await LoadImage();
            var flyWeight = BillboardFlyweightFactory.Create(new(3, 3), image, Color.Red, false, true);
            return new Billboard(pos, flyWeight);
        }
